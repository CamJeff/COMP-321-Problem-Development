...
The first line contains two integers $M$ and $N$, representing the minimum number of points required for full marks and the number of available problems on this assignment. Note that $M$ may be very large (e.g., $M \ge 10^{20}$).

\bigskip

\noindent
The second line contains a list of distinct strings, each representing a topic. The list is ordered from the topic you understand best to the one you understand least.
...
\begin{itemize}
    ...
    \item \texttt{Difficulty} is an integer rating from $1$ to $10$, with larger values indicating harder problems.
    \item \texttt{Topic} is a non-empty string (no spaces) indicating the assigned topic of the problem. Every topic must appear in the topic list on the second line.
    \item \texttt{Text\_Length} is an integer between $1$ and $10^4$ representing the number of words in the problem statement.
\end{itemize}
